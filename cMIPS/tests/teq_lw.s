...
_excp_180:
	...
	mfc0  $k1, c0_epc		# move EPC forward one instruction
	addi  $k1, $k1, 4
	mtc0  $k1, c0_epc
...
main:	la    $15, x_IO_BASE_ADDR # print out address (simulator's stdout)
	la    $16, x_DATA_BASE_ADDR
	##
	## do a trap in a load delay slot
	##
	li   $9, '\n'
	sw   $9, x_IO_ADDR_RANGE($15)     # print out '\n' to separate tests
	li   $3, '!'		# put something on $3 and store it
	sw   $3, 0($16)
	nop
	lw   $5, 0($16)		# load it back
	teq  $0,$0		#  then trap (does not modify $5)
	addi $5, $5, 4		#    then change it to '%'
	nop
	sw   $5, x_IO_ADDR_RANGE($15)  # print out '%'
	li   $9, '\n'
	sw   $9, x_IO_ADDR_RANGE($15)  # print out '%'
	nop
	nop
...
wrong1: sw  $5, x_IO_ADDR_RANGE($15)
	li  $9, 'e'
	sw  $9, x_IO_ADDR_RANGE($15)
	li  $9, 'r'
	sw  $9, x_IO_ADDR_RANGE($15)
